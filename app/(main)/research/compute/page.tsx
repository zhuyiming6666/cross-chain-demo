  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <Button icon="pi pi-plus" label="创建计算任务" onClick={() => router.push("/research/compute/create")} />
      </div>
      <Card>
        {tasks.length === 0 ? (
          <div className="text-center py-12">
            <i className="pi pi-cog text-5xl text-surface-300 mb-3" />
            <p className="text-surface-500 mb-3">暂无计算任务</p>
            <Button icon="pi pi-plus" label="创建第一个任务" onClick={() => router.push("/research/compute/create")} />
          </div>
        ) : (
          <>
            {/* Mobile card list */}
